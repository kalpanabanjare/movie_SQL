...
SELECT COUNT(*) AS Total_No_of_row_director_mapping FROM director_mapping;

select count(*) as total_no_of_row_movie from movie;

select count(*) as total_no_of_row_names from names;

select count(*) as total_no_of_row_ratings from ratings;

select count(*) as total_no_of_row_role_mappimg from role_mapping;
...
select sum(case
        ...
        end) as date_published_count_null,
        sum(case
				when duration is null then 1
				else 0
        end) as duration_count_null,
        sum(case
				when country is null then 1
				else 0
        end) as country_count_null,
        sum(case
				when worlwide_gross_income is null then 1
				else 0
        end) as worlwide_gross_income_count_null,
        sum(case
				when languages is null then 1
				else 0
        end) as languages_count_null,
        sum(case
				when production_company is null then 1
				else 0
        end) as production_company_count_null
from movie;
...
select count(distinct id) as no_of_movies
from movie
...
select genre, count(id) as no_of_movie
from movie as m
inner join genre as g
where g.movie_id = m.id
group by genre
order by no_of_movie desc limit 1;
...
with movie_belongs_one_genre
as (select movie_id
	from genre 
	group by movie_id 
	having count(distinct genre) = 1)
select count(*) as movie_belongs_one_genre
from movie_belongs_one_genre;
...
select genre, count(movie_id) as movie_count , dense_rank()
...
select rating, count(movie_id) as movie_count 
from ratings 
...
select production_company, count(id) as movie_count, dense_rank()
...
SELECT COUNT(m.id) AS no_of_movie_released, r.median_rating 
FROM movie AS m 
INNER JOIN ratings AS r 
ON m.id = r.movie_id 
...
select genre, round(avg(duration)) as avg_duration, sum(avg(duration))
over(order by genre) as running_total_duration,
avg(avg(duration)) over(order by genre) as running_total_duration
from movie as m 
inner join genre as g 
on m.id = g.movie_id
group by genre;
